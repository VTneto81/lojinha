      ...
      <header className="bg-white shadow-sm sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            ...
            <div><span className="text-xl font-extrabold text-blue-600">Lojinha</span><span className="text-xl font-extrabold text-teal-500"> da Mamãe</span></div>
          </Link>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setCartOpen(true)} className="relative bg-blue-600 hover:bg-blue-700 text-white rounded-full p-2 transition-colors">
              ...
              {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-yellow-400 text-xs font-bold text-gray-800 rounded-full w-5 h-5 flex items-center justify-center">{cartCount}</span>}
            </button>
          </div>
        </div>
      </header>
